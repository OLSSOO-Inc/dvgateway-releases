...
- **특수문자/이모지 팔레트** — 이모지·특수문자·장식을 클릭해 본문 커서 위치에 삽입(복사/붙여넣기 불필요).
- **80바이트 제한** — 본문 최대 80바이트(EUC-KR). 초과 입력은 차단되고 `N/80바이트` 카운터로 안내(치환 변수가 있으면 치환 후 길이로 서버가 최종 판정 → 초과 시 `sms_body_too_long`).
