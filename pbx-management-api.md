...
> 이 파일이 없으면 게이트웨이가 **테넌트 기본**(`PUT /api/v1/earlymedia/_default` 로
> 업로드/TTS 합성한 `pa/_default/pamsg.wav`)을 내선 경로로 자동 복사하고, 그것도
> 없으면 `400 {"code":"pea_no_recording"}` 으로 거부합니다(파일 없이 켜면 인입이
> 재생 실패 후 음성사서함으로 빠짐). 내선별 안내음 등록/교체 =
> `PUT /api/v1/earlymedia/{단말번호}` (url/tts/upload).
